...
names(df_results)[names(df_results) == "How.much.experience.do.you.have.with.virtual.reality..VR..and.augmented.reality..AR....VR."] <- "experience(VR)"
names(df_results)[names(df_results) == "How.much.experience.do.you.have.with.virtual.reality..VR..and.augmented.reality..AR....AR."] <- "experience(AR)"
...
if (plot) {
  ...
  pie +
    ...
    scale_fill_manual(values = c("#941c20", "#009269", "#4A5745"))


  ### Plot VR experience
  t <- table(df_results["experience(VR)"])
  g <- c("A1", "A2", "A3", "A4", "A6", "A7")
  v <- c(t[g[1]], t[g[2]], t[g[3]], t[g[4]], t[g[5]], t[g[6]])
  v[is.na(v)] <- 0
  df_plot <- data.frame(
    group = g,
    value = v
